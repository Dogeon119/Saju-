...
| 경로 | 역할 |
|---|---|
| `app/` | 전역 하단 4탭: **홈**(모드 5종 허브 — `/saju` 13장 · `/love` 7장 · `/gunghap` 8장 · `/yearly` 8장 · `/manse` 6장) · **오늘**(`/daily` 4장) · **서재**(`/library` 감정서 보관함) · **회원**(`/account`) + `/admin` 관리자(3중 게이트) · `/r` 공유 · `/i` 궁합 초대 |
| `components/` | 공용 컴포넌트 (SiteHeader · ModeTabs · ReadingApp) + 모드 메타(`modes.ts`) |
| `lib/engine/` | 사주 엔진 — ssaju 어댑터(`analyze.ts`) · 합충/십성(`relations.ts`) · 모드 렌더러(`modes.ts`) · 공용 렌더 헬퍼(`render.ts`) · 일진 위젯 유틸(`today.ts`, ssaju 대조 테스트) |
