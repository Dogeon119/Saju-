...
export default function AccountApp() {
  ...
  return (
    <>
      <div className="acct-row">
        <div>
          ...
          <p className="acct-sub">
            월하 회원{session.user.created_at ? ` · ${new Date(session.user.created_at).getFullYear()}. ${new Date(session.user.created_at).getMonth() + 1}. ${new Date(session.user.created_at).getDate()}. 입회` : ""}
          </p>
        </div>
      ...
      <div className="mode-list" style={{ marginTop: 16, marginBottom: 16 }}>
      ...
      <form onSubmit={onChangePw} noValidate className="card" style={{ padding: 24 }}>
        <p className="card-h">보안 — 비밀번호 변경</p>
        <div className="grid">
          <div className="field full">
