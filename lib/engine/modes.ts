...
const first = (t, n = 1) => t.split(". ").slice(0, n).join(". ") + (t.split(". ").length > n ? "." : "");
...
const ORGANS = ["간·담·눈·근육", "심장·혈관·혀·시력", "위장·비장·피부", "폐·기관지·대장·피부", "신장·방광·귀·뼈"];
const ILGAN_META = ["추진력과 리더십의 사람", "유연함과 적응력의 사람", "표현력과 존재감의 사람", "집중력과 섬세함의 사람", "신뢰와 뚝심의 사람", "서포트와 키우는 힘의 사람", "결단력과 의리의 사람", "안목과 완성도의 사람", "큰 그림과 통찰의 사람", "섬세함과 직관의 사람"];
...
export function renderSaju(A) {
  ...
  /* ── 第一章 : 원국 + 사주의 뼈대(신강신약·용신·조후) — 사주 모르는 사람도 바로 이해되게 ── */
  const basis = [s.deukryeong ? "태어난 달이 내 편이고" : null, s.deukji ? "태어난 날에 뿌리가 있고" : null, s.deukse ? "도와주는 기운이 많아" : null].filter(Boolean);
  const noBasis = [!s.deukryeong ? "태어난 달이 내 편이 아니고" : null, !s.deukji ? "태어난 날의 뿌리가 여리고" : null, !s.deukse ? "도와주는 기운이 적어" : null].filter(Boolean);
  const METHOD_EASY = { "억부(抑扶)": "넘치면 덜고 모자라면 채워 균형을 맞추는 방식", "조후(調候)": "계절의 추위·더위를 다스려 온도를 맞추는 방식", "통관(通關)": "부딪히는 두 기운 사이를 이어 주는 방식", "전왕(專旺)": "가장 센 기운을 거스르지 않고 그 편에 서는 방식" };
  const warmEasy = cl.warmth === "한(寒)" ? "차가운 편" : cl.warmth === "난(暖)" ? "뜨거운 편" : "적당한 편";
  const strengthBody =
    `<p><strong>사주는 '나'를 중심으로 봐요.</strong> 태어난 날의 기운이 곧 '나'인데, 그 '나'의 힘이 센 편인지 여린 편인지 보는 게 <strong>신강·신약</strong>이에요.</p>` +
    `<p>당신은 <strong>${strengthName}</strong> — ${s.cls === "strong"
      ? `${basis.join(", ")} 힘이 강한 편이에요. 추진력·독립심이 강점이라 남 밑보다 <strong>내가 주도하는 자리</strong>(창업·전문직·팀장)가 잘 맞아요. 단점은 '내 방식' 고집이라 부딪힘이 잦은 것 — 넘치는 힘을 일·운동·목표로 <em>써서 빼는</em> 게 균형이에요.`
      : s.cls === "weak"
        ? `${noBasis.join(", ")} 힘이 여린 편이에요. 혼자 다 짊어지기보다 <strong>좋은 판을 고르는 것</strong>이 전략이라, 프리랜서보다 소속, 단독보다 팀·멘토·좋은 회사 위에서 실력이 배가돼요. 거절 못 하고 떠안는 습관만 고치면 훨씬 편해져요.`
        : `힘이 넘치지도 모자라지도 않은 균형형이에요. 상황 따라 밀고 당기는 <strong>조절력</strong>이 강점이라, 이끄는 자리와 맞추는 자리 양쪽에 다 잘 적응해요.`}</p>` +
    kv([["힘의 세기", `${strengthName} · 100점 만점에 ${s.score}점`], ["받쳐 주는 것", `태어난 달 ${s.deukryeong ? "○" : "×"} · 태어난 날의 뿌리 ${s.deukji ? "○" : "×"} · 같은 편 세력 ${s.deukse ? "○" : "×"}`]]) +
    note("쉽게 말하면 '나'라는 사람이 얼마나 튼튼한가예요. 센 사람은 힘을 좀 빼 주고 여린 사람은 채워 줘야 인생이 편해지는데, 그 방법이 바로 다음에 나오는 '용신'이랍니다.");
  const yongBody =
    `<p><strong>용신</strong>은 어렵게 들리지만 뜻은 간단해요 — <strong>나한테 가장 필요한 기운</strong>이에요. 당신에게 필요한 건 <strong>${ELEM[T.yongElem]}</strong>이고요. ${T.yongReason}.</p>` +
    `<p>이건 ${METHOD_EASY[T.yongMethod] ?? "사주의 균형을 맞추는 방식"}으로 찾은 거예요. 이 기운이 곧 나의 행운 포인트라서 — 색은 <strong>${COLOR[T.yongElem]}</strong>, 방향은 <strong>${DIR[T.yongElem]}</strong>, 계절은 ${["봄", "여름", "환절기", "가을", "겨울"][T.yongElem]}이 평생 나를 돕는 부적이 돼요.</p>` +
    kv([["가장 필요한 기운", `${ELEM[T.yongElem]} (용신)`], ["도와주는 기운", `${ELEM[T.heeElem]} (희신)`], ["피해야 할 기운", `${ELEM[T.giElem]} (기신)`]]) +
    note("전문어로 용신·희신·기신이라 부르는데, 그냥 '가장 필요한 기운 / 도와주는 기운 / 피해야 할 기운' 셋이에요. 색·방향·사람·시기를 고를 때 나침반이 되는 값이랍니다.");
  const johuBody =
    `<p>태어난 <strong>계절</strong>도 사주에 크게 작용해요. 당신은 <strong>${cl.season}</strong>에 태어나서, 기운의 온도가 <strong>${warmEasy}</strong>예요.</p>` +
    `<p>${cl.needElem != null
      ? `${cl.season}에 태어난 사람은 ${ELEM[cl.needElem]} 기운이 돌아야 잘 풀려요. ${cl.critical ? `지금 그게 부족해서 <strong>계절 온도부터 맞추는 게 급한 사주</strong>예요 — ${ELEM[cl.needElem]}을 채우는 색·음식·사람이 곧 행운이에요.` : `${ELEM[cl.needElem]}을 곁에 두면 사주가 한결 부드럽게 흘러요.`}`
      : `계절의 춥고 더움이 극단으로 치우치지 않아 편안한 구조예요. 앞에서 본 '힘의 균형'에만 신경 쓰면 돼요.`}</p>`;
  html += sec("第一章", "나의 사주팔자", `${A.y}년 ${A.m}월 ${A.d}일생 · ${BRANCHES[r.pillarDetails.year.branchIdx].animal}띠 · ${r.currentAge}세 · ${strengthName}`,
    baseChartSec(A) +
    grp("사주의 큰 그림 — 딱 세 가지만 보면 돼요") +
    fl(1, "① 내 기운이 센 편일까, 여린 편일까", strengthBody) +
    fl(2, "② 나에게 가장 필요한 기운은?", yongBody) +
    fl(3, "③ 태어난 계절이 준 기질", johuBody),
    `${strengthName} · 필요한 기운은 ${ELEM[T.yongElem]} · ${cl.season}생`);

  html += sec("第二章", "일주와 오행 분석", `일주 ${gzName(ds, db)} · ${T.dayRooted ? "뿌리 단단" : "뿌리 여림"}`,
    paras(ILGAN_CHAR[ds]) +
    `<p><strong>일주로 좁혀 볼게요.</strong> 태어난 날의 하늘 기운 ${STEMS[ds].kr}(${STEMS[ds].hj})가 땅 기운 ${BRANCHES[db].kr}(${BRANCHES[db].hj}) 위에 앉은 것 — 이게 사주에서 '나'를 뜻하는 일주(日柱)예요. ${T.rootNote}.</p>` +
    `<p><strong>오행(다섯 기운)의 저울</strong>이에요. 겉글자만 세지 않고 땅 기운 속에 숨은 기운(지장간)까지 넣어 무게를 달면 <strong>${ELEM[maxE]} ${T.powerPct[maxE]}%</strong>로 가장 크고 <strong>${ELEM[minE]} ${T.powerPct[minE]}%</strong>로 가장 작아요 — 이게 진짜 기운의 세기랍니다.</p>` +
  ...
  html += sec("第六章", "귀인 분석", "나를 돕는 하늘의 손",
    gilsin.length
      ...
      : `<p>원국에 드러난 귀인성이 강하진 않아요. 그런데 이건 도움이 없다는 뜻이 아니라, <strong>당신의 귀인은 하늘이 아니라 당신이 쌓은 신뢰에서 나온다</strong>는 뜻이에요. 스스로 만든 인복이 가장 오래가는 법이거든요.</p>`,
  ...
  html += sec("終章", "월하노인에게 묻다 — 세 가지 문답", "가장 많이 받는 질문 셋, 당신 사주로 답해 드릴게요",
    ...
    `<div class="qa"><p class="q">Q3. 나의 개운법은 뭔가요?</p><p class="a">당신의 용신은 <strong>${ELEM[yongE]}</strong>이에요. 색은 ${COLOR[yongE]}, 방향은 ${DIR[yongE]}이 길해요. 중요한 약속·면접·계약 때 이 방향과 색을 곁에 두세요.</p></div>`,
...
export function renderLoveSecret(A, relStatus = 0, relGap = 0) {
  ...
  html += sec("第一章", "나만의 매력은?", `일주 ${gzName(ds, db)}`,
    baseChartSec(A) +
    fl(1, "내가 타고난 매력",
      (ch.length ? `<div class="badges">${ch.map(c => `<span class="badge">${c}</span>`).join("")}</div><p>매력의 별을 타고나셨어요 — ${ch.map(c => SAL_DESC[c].split("—")[0].trim()).join(", ")}. 여기에 일간의 매력까지 얹혀요: ${STRENGTHS[ds][0]}.</p>`
        : `<p>화려한 신살보다 <strong>알수록 빠져드는 유형</strong>의 매력이에요. 당신의 무기는 ${STRENGTHS[ds][0]} — 오래 겪을수록 진가가 드러나는 타입이라, 노출 시간이 곧 매력 자본이랍니다.</p>`)) +
    fl(2, "이성이 느끼는 나의 첫인상", FIRST_IMPRESSION[ds]) +
    fl(3, "연애할 때 빛나는 장점과 보완할 점",
      `<p><strong>빛나는 장점</strong>부터요.</p><ul class="pts blue">${STRENGTHS[ds].map(s => `<li>${s}</li>`).join("")}</ul>` +
      `<p>그리고 솔직하게, <strong>보완할 점</strong>도요.</p><ul class="pts">${CAUTIONS[ds].map(s => `<li>${s}</li>`).join("")}</ul>`),
    DAY_KEY[ds]);
  ...
  html += sec("第二章", "연애운 흐름과 시기", "평생의 결부터 앞으로 3년까지",
    fl(1, "평생의 연애운 특징", `<p>${first(DAY_LOVE[ds], 3)}</p><p><strong>배우자궁</strong>은요 — ${SP_KEY[db]}.</p>`) +
    fl(2, "앞으로 3년간 다가올 연애",
    ...
    fl(3, "인연이 찾아오는 때는?",
      `<p>가장 큰 문은 <strong>${best.y}년 ${best.gz}년</strong>이에요.${goodM.length ? ` 달로는 <strong>${goodM.map(m => m + "월").join(" · ")}</strong>이 길하고요 — 이 달의 소개, 모임, 재회 연락은 흘려보내지 마세요.` : ""}</p>`) +
    fl(4, "연애하면 안 되는 시기",
      `<p>${badM.length ? `<strong>${badM.map(m => m + "월").join(" · ")}</strong>은 배우자궁이 흔들리는 충의 달이에요 — 새 인연의 시작이나 중요한 관계 결정(고백·이별 통보)은 피하는 게 좋아요.` : "올해는 크게 피할 달이 없어요."}${giYears.length ? ` 그리고 ${giYears.map(v => v.y + "년").join("·")}은 기신(${ELEM[giE]})이 힘을 얻는 해라, 판단이 흐려진 채로 시작하는 연애는 조심해 주세요.` : ""}</p>`),
    `가장 큰 문: ${best.y}년${goodM.length ? ` · 길한 달 ${goodM.slice(0, 2).map(m => m + "월").join("·")}` : ""}`);

  html += sec("第三章", "연애운 상승 방법은?", "경로 · 아이템 · 습관",
    fl(1, "나에게 잘 맞는 인연 진입 경로",
      [3, 6, 9, 0].includes(db)
        ? "당신의 배우자궁은 도화의 자리예요 — 사람 많은 곳에서 시선이 먼저 도착하는 유형이죠. 모임, 행사, 동호회처럼 여럿이 모이는 판이 최적의 진입 경로고, 오히려 일대일 소개팅에선 매력이 반감돼요."
        : "당신의 배우자궁은 차분한 자리예요 — 북적이는 곳보다 신뢰의 다리를 건너오는 인연이 맞아요. 지인 소개, 스터디, 업무 접점처럼 서로의 평판이 보이는 경로가 최적이랍니다.") +
    fl(2, "나만의 연애운 상승 아이템",
      `<p>용신이 ${ELEM[yongE]}이니까 <strong>${COLOR[yongE]}</strong> 계열의 소품(옷·폰케이스·액세서리)이 당신의 부적이에요. ${first(LUCK_TIPS[pe], 2)}</p>`) +
    fl(3, "행복한 연애를 막는 무의식적 습관과 개선법",
      `<p><strong>무의식적 습관</strong> — ${CAUTIONS[ds][0]}. 연애가 반복해서 같은 지점에서 어긋난다면 십중팔구 이거예요.</p><p><strong>개선법</strong> — 당신은 ${RECONCILE[ds]}</p>`),
    `${COLOR[yongE]} 아이템 + 습관 하나 교정 = 연애운 상승의 전부`);

  const meetScene = [
    "배우거나 성장하는 자리 — 학원·스터디·강연·자기계발 모임에서 옆자리 인연으로 올 확률이 높아요. 새로 뭔가 시작하는 시기에 특히요.",
    "사람 많고 활기찬 자리 — 모임·행사·파티·취미 동호회에서 눈이 마주치며 시작돼요. 웃음소리 큰 자리를 피하지 마세요.",
    "일상의 신뢰가 쌓이는 자리 — 직장·오래 다닌 동네·지인 소개처럼 '검증된' 경로로 와요. 요란한 첫 만남보다 서서히 스며들어요.",
    "실력이나 일로 엮이는 자리 — 업무·프로젝트·전문 모임에서 '일 잘한다'는 인상으로 서로를 알아보게 돼요.",
    "조용하고 깊은 자리 — 밤늦은 카페, 소수의 진지한 모임, 온라인의 깊은 대화에서 스며들듯 시작돼요.",
  ][pe];
  const drawn = [
    "함께 있으면 뭔가 계획이 서고 앞으로 나아가는 느낌 — 미래가 그려지는 사람이라 끌려요.",
    "같이 있으면 분위기가 환해지고 지루할 틈이 없어요 — 내 에너지를 채워 주는 사람이라 끌려요.",
    "곁에 있으면 이상하게 마음이 놓이고 편안해요 — 기댈 수 있는 안정감이 가장 큰 매력이에요.",
    "선이 분명하고 흐트러짐이 없어서, 볼수록 '이 사람은 믿을 만하다'는 신뢰가 생겨요.",
    "속을 다 보여 주지 않아 자꾸 궁금해지는 사람 — 대화할수록 그 깊이에 빠져들어요.",
  ][pe];
  const datingVibe = [
    "약속을 잘 지키고 관계를 '키워 가는' 스타일이라, 같이 목표를 세우고 이뤄 가는 연애를 해요.",
    "표현이 확실하고 이벤트에 진심이라 설렘이 오래가요. 대신 기복이 있으니 리듬만 맞춰 주면 돼요.",
    "요란하진 않아도 꾸준히 챙기는 스타일이라 편안하고 오래가는 연애예요. 무심해 보여도 속정이 깊어요.",
    "선을 지키며 존중하는 반듯한 연애예요. 표현이 담백해 서운할 수 있지만, 한 약속은 꼭 지키는 사람이에요.",
    "깊은 대화로 마음을 나누는 연애예요. 감정을 안으로 삭이는 편이라, 먼저 물어봐 주면 활짝 열려요.",
  ][pe];
  const watchOut = [
    "고집이 셀 수 있어요 — 방향이 정해지면 잘 안 꺾이니, 정면 반박보다 '이런 방법도 있어'로 돌려 말해 주세요.",
    "감정 기복이 있어요 — 뜨거웠다 식었다 하니, 그 순간 기분을 사실처럼 받아들이지 말고 하루 뒤에 다시 보세요.",
    "표현이 느리고 변화를 싫어해요 — 무심한 게 아니라 원래 그런 거예요. 답답할 땐 콕 집어 요청하면 잘 해 줘요.",
    "옳은 말을 차갑게 해서 상처를 줄 때가 있어요 — 원칙이 강한 만큼, 감정 호소보다 논리로 다가가면 통해요.",
    "혼자 생각에 잠겨 벽을 칠 때가 있어요 — 다그치면 더 닫히니, 시간을 주고 편하게 열어 주세요.",
  ][pe];
  const howTo = [
    "먼저 '요즘 뭐 준비해?'처럼 미래·계획을 물어보세요. 눈이 반짝이면 그 사람이에요. 응원해 주는 게 최고의 어필이고요.",
    "리액션 크게, 같이 즐거워해 주세요. 이 유형은 '나랑 있으면 재밌는 사람'에게 마음이 열려요.",
    "급하게 밀어붙이지 말고 자주 얼굴 비추며 신뢰를 쌓으세요. 챙겨 주고 밥 잘 사 주는 것에 약해요.",
    "약속·시간 잘 지키고 깔끔한 모습을 보여 주세요. 이 유형은 '믿을 만한 사람'에게 끌려요.",
    "가벼운 잡담보다 진짜 대화를 나눠 보세요. 내 속마음을 먼저 조금 열면 상대도 따라 열어요.",
  ][pe];
  html += sec("第四章", "내 운명의 짝은?", `${sName} = ${ELEM[pe]} 기운의 사람`,
    `<p><strong>사주가 가리키는 내 짝은 「${ELEM[pe]} 기운」의 사람이에요.</strong> ${A.sex === "F" ? "나를 이끌고 지켜 주는 짝" : "내가 아끼고 챙기고 싶어지는 짝"}의 기운이 사주에서 이 ${ELEM[pe]}으로 나오거든요. 어떤 사람인지, 어디서 만날지, 어떻게 다가갈지 하나씩 그려 볼게요.</p>` +
    fl(1, "생김새와 첫인상",
      kv([
    ...
    fl(2, "성격과 살아가는 결",
      kv([
        ...
        ["잘 어울리는 일", ["교육·기획·출판·의료·스타트업", "예술·방송·마케팅·요식·공연", "부동산·금융·행정·건설", "법률·회계·IT·의료·군경", "연구·데이터·글·무역·상담"][pe]],
        ["잘 맞는 계절·방향", `${["봄", "여름", "환절기", "가을", "겨울"][pe]} · ${DIR[pe]}`],
      ]) + paras(prof.mind)) +
    fl(3, "어디서, 어떻게 만나게 될까",
      paras(prof.where) + `<p>${meetScene}</p>`) +
    fl(4, "왜 이 사람에게 끌릴까", `<p>${drawn}</p>`) +
    fl(5, "이 사람과 연애하면 이런 그림", `<p>${datingVibe}</p>`) +
    fl(6, "이런 점은 미리 알아 두세요", `<p>${watchOut}</p>`) +
    fl(7, "알아보는 법 & 다가가는 팁", `<p>${howTo}</p>` +
      note(`전문어로는 이 짝의 기운을 ${A.sex === "F" ? "관성(官星)" : "재성(財星)"}이라 불러요. 겁먹지 마세요 — 그냥 '내 짝을 뜻하는 기운' 정도로 아시면 충분해요.`)),
  ...
  html += sec("第五章", "운명일 줄 알았는데, 아닌 사람", `기신(忌神) ${ELEM[giE]}의 악연`,
    fl(1, "인연인 척하는 악연의 모습",
      `<p>${first(giProf.look, 2)} 이렇게 매력적으로 보이는데요 — 당신의 균형을 무너뜨리는 <strong>기신(${ELEM[giE]})</strong>의 기운을 강하게 두른 사람이에요.</p><p>주로 ${first(giProf.where, 1)}</p>`) +
    fl(2, "어떤 사람을 조심해야 하는지",
      `<p>${YONGSIN_FRAME.gi}</p><p>구체적으로는 — ${ELEM_PERSON[giE]} 중에서, 만나고 오면 이상하게 <strong>지치고 자신이 없어지는 사람</strong>이요. 그게 악연의 가장 확실한 신호거든요.</p>`) +
    fl(3, "착각하게 되는 상황",
      `<p>도화가 발동하는 ${monthsWhere(A, mb => mb === DOHWA[db]).map(m => m + "월").join("·") || "시기"}, 그리고 외로움이 깊어진 밤 — 이때의 설렘은 인연 감별력이 가장 낮은 상태에서 와요. 이 시기의 '운명 같다'는 느낌은 하루 재우고 다시 보세요.</p>`) +
    fl(4, "맞지 않는 사람을 빨리 알아보는 법",
      `<ul class="pts">
        <li>만난 뒤의 내 상태를 보세요 — 충전되면 인연, 방전되면 악연이에요.</li>
        <li>약속의 무게를 보세요 — 말이 크고 실행이 작은 사람은 ${ELEM[giE]} 기신의 전형이에요.</li>
        <li>내 사람들에게 소개할 수 있는지 스스로에게 물어보세요 — 망설여진다면, 몸이 먼저 알고 있는 거예요.</li>
      </ul>`),
  ...
  html += sec("第六章", "지금 나의 연애 상태 처방", `${REL_STATUS[relStatus]}${relStatus > 0 ? ` · ${REL_GAP[relGap]}` : ""}`,
    ...
    (relStatus > 0 ? `<p><strong>기간의 의미</strong>도 볼게요 — ${REL_GAP[relGap]}이라는 시간은 ${["아직 관성이 생기기 전이라 방향을 바꾸기 가장 쉬운 시기", "패턴이 자리 잡는 시기예요. 지금의 습관이 관계의 기본값이 되죠", "안정과 권태가 교차하는 시기라, 새로움을 의식적으로 공급해야 해요", "생활이 된 시기예요. 관계의 온도는 이벤트가 아니라 존중의 습관이 지켜요"][relGap]}${relGap === 0 ? "예요" : ""}.</p>` : ""),
...
export function renderGunghapDeep(A, B, relStatus = 3) {
  ...
  const personBlock = (P, who, yT) => {
    ...
    return grp(`${who}의 사주팔자 분석`) +
      ...
      fl(2, "나만의 매력은?", `${FIRST_IMPRESSION[P.ds]}. 결정적 한 방은 <strong>${STRENGTHS[P.ds][0]}</strong>이에요.`) +
      fl(3, "내가 꿈꾸는 이상형은?", `${ELEM[ipe]} 기운의 사람이에요 — ${first(LOVER_PROFILE[ipe].look, 1)}`) +
      fl(4, "내가 중시하는 연애 가치관은?", VALUES_TG[dom] ?? "균형 잡힌 관계를 원해요") +
      fl(5, "연애에서 보이는 나의 단점은?", `${CAUTIONS[P.ds][0]}, 그리고 ${CAUTIONS[P.ds][1]} — 이 두 가지예요.`) +
      grp(`${who}의 오행 분석`) +
      ohaengHTML(P.elems) +
      fl(1, "오행 분석", `${ELEM[P.elems.indexOf(Math.max(...P.elems))]}이 가장 강하고 ${ELEM[P.elems.indexOf(Math.min(...P.elems))]}이 가장 약한 구조예요.`) +
      fl(2, "용신(用神)", `${YONGSIN_FRAME.yong} ${who}의 용신은 <strong>${ELEM[yT.yongE]}</strong> — 색은 ${COLOR[yT.yongE]}, 방향은 ${DIR[yT.yongE]}이에요.`) +
      fl(3, "희신(喜神)", `${YONGSIN_FRAME.hee} ${who}의 희신은 <strong>${ELEM[yT.heeE]}</strong>이에요.`) +
      fl(4, "기신(忌神)", `${YONGSIN_FRAME.gi} ${who}의 기신은 <strong>${ELEM[yT.giE]}</strong>이에요.`) +
      grp(`${who}의 신강신약 분석`) +
  ...
  html += sec("第二章", "인연궁합", "붉은 실의 정체를 확인해 볼게요",
    grp("1. 인연의 붉은 실") +
    fl(1, "우리는 인연일까, 악연일까?",
    ...
    fl(2, "전생에서 우리는 어떤 관계였을까?", `${PAST_LIFE[tg]}. 그래서 이번 생의 ${B.name}님은 ${A.name}님에게 ${tg}의 별로 다시 온 거예요.`) +
    grp("2. 인연과 관계의 열쇠") +
    fl(1, "서로가 느꼈던 첫인상은?", `${A.name}님이 본 ${B.name}님: ${FIRST_IMPRESSION[B.ds]}. 반대로 ${B.name}님이 본 ${A.name}님: ${FIRST_IMPRESSION[A.ds]}.`) +
    fl(2, "고백은 누가 먼저 하는 게 좋을까?", `<strong>${firstConfess}님</strong>이 먼저 움직이는 그림이 자연스러워요. ${aControls || bControls ? "상대를 향해 기운이 흐르는 쪽이 다가갈 때 관계가 순리대로 풀리거든요." : "양(陽)의 기운이 강한 쪽이 문을 여는 게 명리의 순서예요."}`) +
    fl(3, "계절별 추천 데이트 & 여행 코스",
      `<ul class="pts gold"><li><strong>${A.name}님의 용신 계절</strong> — ${SEASON_DATE[yA.yongE]}</li><li><strong>${B.name}님의 용신 계절</strong> — ${SEASON_DATE[yB.yongE]}</li><li>두 계절을 번갈아 채우면 서로의 운을 데워 주는 데이트가 돼요.</li></ul>`) +
    fl(4, "기념일은 어떻게 챙기는 게 좋을까?", `${A.name}님은 ${msA.event}. ${B.name}님은 ${msB.event}. 기준이 서로 다르니까 '금액'이 아니라 '방식'을 먼저 맞춰 보세요.`) +
    fl(5, "연애 시기별로 마음을 더 주는 쪽은?", `초반의 불은 <strong>${earlyGiver}님</strong>이 크게 지피고, 관계가 익을수록 <strong>${lateGiver}님</strong>의 꾸준함이 저울을 잡아요.`) +
    fl(6, "서로 어떤 점에서 서운함을 느낄까?", `${A.name}님은 「${CAUTIONS[B.ds][0]}」에서, ${B.name}님은 「${CAUTIONS[A.ds][0]}」에서 서운함이 쌓이기 쉬워요. 서로의 아킬레스건, 미리 알아 두세요.`) +
    fl(7, "화해는 어떻게 하면 좋을까?", `${A.name}님은 ${RECONCILE[A.ds]}<br>${B.name}님은 ${RECONCILE[B.ds]}`) +
    fl(8, "장기적인 관계 유지를 위한 조언", `${BR_KEY[br.k]}. 그리고 두 사람 공용의 만능 처방 하나 — 갈등이 생긴 날 잠들기 전에 "그래도 네 편이야" 한 문장. 어떤 합(合)보다 강해요.`),
  ...
  html += sec("第三章", "성격궁합", "결이 맞는지, 맞춰 갈 수 있는지",
    grp("1. 우리의 성격, 잘 맞을까?") +
    fl(1, "일간으로 보는 성격의 조화", `<p>${TG_GUNGHAP[tg][0]}</p><p>${first(TG_GUNGHAP[tg][1].replace(/<[^>]+>/g, ""), 3)}</p>`) +
    fl(2, "연애할 때 달라지는 모습은?", `${A.name}님은 「${DAY_KEY[A.ds].split("—")[0].trim()}」의 연애를, ${B.name}님은 「${DAY_KEY[B.ds].split("—")[0].trim()}」의 연애를 해요.`) +
    fl(3, "자연스럽게 나눠지는 역할은?", `끌고 가는 쪽은 <strong>${leader}님</strong>, 살림과 안정을 챙기는 쪽은 <strong>${lateGiver}님</strong>의 그림이 자연스러워요. 역할이 겹치는 날 다툼이 나니까, 각자의 영역을 인정해 주세요.`) +
    fl(4, "주변이 부러워할 우리 커플만의 장점", `${BR_KEY[br.k].split("—")[1]?.trim() ?? "서로를 있는 그대로 보는 맑음"} — 그리고 ${STRENGTHS[A.ds][0]}(${A.name})과 ${STRENGTHS[B.ds][0]}(${B.name})의 합작이에요.`) +
    grp("2. 우리의 끌림 포인트는?") +
    fl(1, "누가 이성에게 더 인기가 많을까?", chA === chB ? "매력의 별 개수가 같아요 — 인기 유형이 다를 뿐 총량은 비등해요." : `${chA > chB ? A.name : B.name}님 쪽이 도화·홍염 같은 매력의 별이 더 많아서 시선을 끄는 힘이 세요. 질투가 아니라 관리의 문제로 다뤄 주세요.`) +
    fl(2, `${B.name}님이 ${A.name}님에게 느끼는 매력`, `${STRENGTHS[A.ds][0]}, 그리고 ${STRENGTHS[A.ds][1]}이에요.`) +
    fl(3, `${A.name}님이 ${B.name}님에게 느끼는 매력`, `${STRENGTHS[B.ds][0]}, 그리고 ${STRENGTHS[B.ds][1]}이에요.`) +
    grp("3. 이 커플, 오래 갈 수 있을까?") +
    fl(1, "오래 만날 수 있는 성격일까?", score >= 70 ? "배합상 지구력이 좋은 조합이에요. 큰 이변이 없는 한 시간이 두 사람의 편이에요." : score >= 50 ? "기본 배합은 무난한데, 자동으로 오래가는 조합은 아니에요. 아래 보완법이 실질적인 수명 연장 장치가 될 거예요." : "솔직하게 말씀드리면 — 배합의 자력은 약한 편이에요. 그런데요, 노력한 커플이 배합 좋은 커플을 이기는 경우를 명리는 수없이 봐 왔답니다.") +
    fl(2, "더 많이 양보해야 할 사람은?", `<strong>${yielder}님</strong>이에요. 기운이 더 넉넉한 쪽(신강)이 반 발 물러설 때 관계 전체의 균형이 잡히거든요.`) +
    fl(3, "연애 주도권은 누구에게?", `${leader}님 쪽으로 자연스럽게 기울어요. 다만 주도권은 '결정권'이 아니라 '먼저 챙길 책임'으로 쓸 때만 복이 돼요.`) +
    fl(4, "미래를 그릴 때 두 사람이 상상하는 그림은?", `${A.name}님의 그림: ${VALUES_TG[dominantTG(A)]}. ${B.name}님의 그림: ${VALUES_TG[dominantTG(B)]}. 두 그림이 다르다면 — 다른 게 아니라 서로의 퍼즐 조각이에요.`) +
    grp("4. 성격 궁합을 보완하는 방법") +
    fl(1, "성격 궁합 보완법", br.k === "chung" || br.k === "wonjin" ? "부딪히는 배합이니까 '고치기'는 포기하고 '규칙'으로 합의하세요. 연락 빈도, 데이트 방식, 돈 문제 — 세 가지만 문서화해도 갈등의 8할이 사라져요." : "무난한 배합이라 최대의 적은 방심이에요. 한 달에 한 번, 서로에게 바라는 점 하나씩만 교환해 보세요.") +
    fl(2, "상대방이 나를 더 좋아하게 만들려면?", `${B.name}님의 용신은 ${ELEM[yB.yongE]}이에요 — ${COLOR[yB.yongE]} 계열을 입고 만나고, ${DIR[yB.yongE]} 방향의 장소를 고르면 이유 모를 호감이 붙는답니다.`) +
    fl(3, "어떤 커플 아이템이 좋을까?", `두 사람의 용신을 하나씩 담아서 — ${COLOR[yA.yongE]} × ${COLOR[yB.yongE]} 조합의 소품(팔찌·폰스트랩·키링)이 두 사람 모두의 기운을 살려 줘요.`) +
    fl(4, "같이 즐기면 좋은 취미", `${SPORT_GOOD[yA.yongE]} 계열(${A.name}님 용신)과 ${SPORT_GOOD[yB.yongE]} 계열(${B.name}님 용신)을 번갈아 해 보세요 — 몸을 함께 쓰는 취미가 이 배합의 정체기를 막아 줘요.`) +
  ...
  html += sec("第四章", "감정궁합", "마음의 온도와 습도",
    ...
    fl(1, "감정 표현 방식", `${A.name}님은 ${EXPRESS[maxA]}. ${B.name}님은 ${EXPRESS[maxB]}.`) +
    fl(2, "대화 스타일", `${A.name}님은 ${TALK[maxA]} ${B.name}님은 ${TALK[maxB]}`) +
    fl(3, "공감 방식", wetA >= wetB ? `${A.name}님이 감정을 받아 주는 그릇이 크고, ${B.name}님은 해결책으로 공감하는 유형이에요. "해결 말고 그냥 들어 줘"라는 신호를 정해 두면 어긋남이 사라져요.` : `${B.name}님이 감정을 받아 주는 그릇이 크고, ${A.name}님은 해결책으로 공감하는 유형이에요. "해결 말고 그냥 들어 줘"라는 신호를 정해 두면 어긋남이 사라져요.`) +
    fl(4, "의존 경향", `기운이 여린 쪽(신약)인 <strong>${clingy}님</strong>이 정서적으로 좀 더 기대는 구도예요. 그런데 기댐은 약점이 아니라 이 관계의 접착제예요 — 기대는 쪽은 표현으로 갚아 주세요.`) +
    fl(5, "내 본능이 상대에게 느낀 '이것'", br.k === "yukhap" ? "살결의 편안함이에요 — 육합의 배합은 곁에 있는 것만으로 안정 호르몬이 도는 사이거든요." : br.k === "chung" || br.k === "wonjin" ? "설명 안 되는 강렬함이에요 — 부딪히는 배합일수록 자력은 세거든요. 본능은 이미 알고 시작한 거예요." : "익숙한 안정감이에요 — 처음부터 오래 안 사이 같았던 그 느낌, 그게 이 배합의 정체예요.") +
    fl(6, "서로에게 안정감을 주는 포인트", `${A.name}님에게는 ${B.name}님의 「${STRENGTHS[B.ds][2]}」이, ${B.name}님에게는 ${A.name}님의 「${STRENGTHS[A.ds][2]}」이 안전지대예요.`) +
    fl(7, "감정적인 순간, 어떻게 행동해야 할까?", `${A.name}님이 격해졌을 때: ${first(RECONCILE[A.ds])} ${B.name}님이 격해졌을 때: ${first(RECONCILE[B.ds])}`) +
    grp("2. 감정 문제의 해결") +
    fl(1, "연락 패턴과 빈도는?", (A.elems[1] + B.elems[1]) >= (A.elems[4] + B.elems[4]) ? "화(火)가 강한 커플이에요 — 짧아도 자주가 정답이에요. 아침저녁 두 번의 가벼운 신호가 장문 하나보다 나아요." : "수(水)가 흐르는 커플이에요 — 빈도보다 깊이예요. 하루 한 번이라도 '오늘의 진짜 기분'을 나누는 대화가 관계를 지켜 줘요.") +
    fl(2, "권태기를 슬기롭게 넘기는 방법", `권태는 감정의 고장이 아니라 <strong>새로움의 결핍</strong>이에요. 두 사람의 용신 계절(${SEASON_DATE[yA.yongE].split("—")[0].trim()}·${SEASON_DATE[yB.yongE].split("—")[0].trim()})마다 처음 가는 장소 하나 — 이 규칙 하나면 충분해요.`) +
    fl(3, "서로 상처받지 않고 대화하는 방법", `${maxA === 3 || maxB === 3 ? "금(金) 기운이 강해서 옳은 말이 아픈 말이 되기 쉬운 조합이에요. " : ""}공식은 하나예요 — <strong>사실 → 감정 → 부탁</strong>의 순서. "늦었네(사실), 걱정했어(감정), 다음엔 미리 알려 줘(부탁)."`) +
    fl(4, "헤어질 위기엔 누가 더 매달릴까?", `<strong>${clingy}님</strong>일 가능성이 높아요. 그러니 상대는 그 마음을 이기는 카드가 아니라 지켜야 할 신뢰로 다뤄 주세요.`) +
    fl(5, "재회를 위해서는 어떤 노력이?", "이 배합이 헤어진다면 원인은 대개 위의 '서운함 포인트'를 방치해서예요. 재회의 조건은 딱 하나 — 같은 문제의 재발 방지책을 먼저 내미는 거예요."),
    `표현·대화·공감의 결이 다른 만큼 — 신호와 공식으로 잇기`);

  html += sec("第五章", "체질궁합", "같이 건강해지는 배합인지",
    grp("1. 우리는 잘 맞는 체질일까?") +
    fl(1, "두 사람의 체질 성향과 주의사항", `${A.name}님: ${first(HEALTH_BY_ELEM[maxA], 2)} ${B.name}님: ${first(HEALTH_BY_ELEM[maxB], 2)}`) +
    fl(2, "상대가 보완해 주는 건강 요소", B.elems[minA] >= 2 ? `${B.name}님의 사주에 ${A.name}님의 부족한 ${ELEM[minA]} 기운이 넉넉해요 — 함께 있는 것만으로 생활 리듬이 보완되는 배합이에요.` : `두 사람 모두에게 부족한 기운이 있으니, 보완은 사람이 아니라 습관(${FOODS[minA]}, ${SPORT_GOOD[minA].split("·")[0]})으로 채워 주세요.`) +
    fl(3, "함께하면 좋은 운동", `${SPORT_GOOD[yA.yongE]} (${A.name}님 용신 기준) / ${SPORT_GOOD[yB.yongE]} (${B.name}님 용신 기준) — 겹치는 것부터 시작해 보세요.`) +
    fl(4, "주의해야 할 운동", `${SPORT_BAD[yA.giE]} (${A.name}님) / ${SPORT_BAD[yB.giE]} (${B.name}님)`),
    `체질은 다를수록 보완 — 함께 몸을 쓰는 커플이 오래가요`);
  ...
  html += sec("第六章", "재물궁합", "사랑은 마음으로, 살림은 구조로",
    ...
    fl(1, `${A.name}님의 재물운`, first(WEALTH_TXT[mCntA === 2 ? 0 : mCntA === 1 ? 2 : 1].split("—")[1] ?? WEALTH_TXT[1], 2)) +
    fl(2, `${B.name}님의 재물운`, first(WEALTH_TXT[mCntB === 2 ? 0 : mCntB === 1 ? 2 : 1].split("—")[1] ?? WEALTH_TXT[1], 2)) +
    fl(3, "함께했을 때의 재물운", mCntA !== mCntB ? "버는 유형과 지키는 유형이 섞인 조합이에요 — 명리학이 가장 반기는 재물 배합이죠. 역할만 제대로 나누면 1+1이 3이 돼요." : "돈에 대한 감각이 닮은 조합이에요. 편한데, 같은 구멍(둘 다 쓰거나 둘 다 묶어두거나)이 생겨요. 외부 규칙(자동이체·상한선)으로 보완하세요.") +
    fl(4, "우리는 재물 걱정 없이 살 수 있을까?", score >= 65 ? "배합상 재물의 그릇이 안정적인 커플이에요. 큰 부는 시기의 문제고, 궁핍의 그림은 안 보여요." : "재물의 안정은 배합보다 규칙에 달린 커플이에요. 아래 상세 풀이의 규칙 세 가지만 지키면 걱정의 8할이 사라져요.") +
    grp("2. 재물궁합 상세 풀이") +
    ...
    fl(3, "데이트 비용은 어떻게 분담하면 좋을까?", mCntA === mCntB ? "성향이 같으니까 정률제(예: 공동 지갑에 같은 비율)가 잡음이 없어요." : "성향이 다르니까 항목제가 나아요 — 계획 지출(식사·여행)은 꼼꼼한 쪽이, 즉흥 지출(선물·이벤트)은 화끈한 쪽이 맡는 식으로요.") +
    fl(4, "비용 갈등을 줄이려면?", "금액을 다투지 말고 '한도'를 합의하세요. 월 데이트 예산 상한 하나만 정해도, 돈 얘기가 감정 얘기로 번지지 않아요.") +
    fl(5, "결혼하면 맞벌이? 외벌이?", bothWork ? "두 사람 다 사회 활동의 별이 살아 있는 사주예요 — 맞벌이가 재물뿐 아니라 관계의 균형에도 좋아요." : "한쪽이 기반을 다지고 한쪽이 살림의 축을 잡는 그림도 잘 어울리는 배합이에요. 다만 '누가 벌든 돈은 공동'이라는 원칙은 문서로 남겨 두세요.") +
    fl(6, "사업을 같이 해도 괜찮을까?", hasGeopjae ? "솔직하게요 — 겁재의 기운이 있어서 동업은 신중해야 해요. 하더라도 지분·정산·철수 조건을 계약서로 만든 다음에만요." : (br.k === "samhap" || br.k === "yukhap" ? "합(合)의 배합이라 공동 프로젝트에 강한 커플이에요. 작은 것(사이드 프로젝트)부터 검증하며 키워 보세요." : "가능은 한데 권장 순서가 있어요 — 여행 예산 운영 → 공동 저축 → 그다음이 사업이에요.")) +
    fl(7, "돈 관리는 누가 하는 게 좋을까?", `<strong>${managerName}님</strong>이에요. ${managerName === A.name ? msA.manage : msB.manage}`) +
    grp("3. 돈에 대한 생각") +
    fl(1, "돈에 대한 우리의 가치관", `${A.name}님에게 돈은 「${VALUES_TG[dominantTG(A)].split("—")[0].trim()}」을 지키는 도구고, ${B.name}님에게는 「${VALUES_TG[dominantTG(B)].split("—")[0].trim()}」을 이루는 수단이에요.`) +
    fl(2, "각자 우선하는 소비 항목", `${A.name}님은 ${["자기계발·계획된 큰 지출", "경험·모임·보이는 것", "생활·저축·실용", "품질·브랜드·자기관리", "취미·지식·조용한 사치"][maxA]}, ${B.name}님은 ${["자기계발·계획된 큰 지출", "경험·모임·보이는 것", "생활·저축·실용", "품질·브랜드·자기관리", "취미·지식·조용한 사치"][maxB]} 쪽으로 지갑이 열려요.`) +
    fl(3, "우리에게 맞는 재테크", `${A.name}님: ${msA.invest} ${B.name}님: ${msB.invest}`) +
    fl(4, "돈 문제가 생겼을 때 대화법", "숫자 얘기는 감정이 낮은 시간(식후·주말 오전)에 하시고, '누가 잘못했나'가 아니라 '다음 달 규칙'으로 끝내세요. 규칙 없는 반성은 반복되거든요."),
  ...
  html += sec("第七章", "혼인궁합", "결혼까지 내다본 두 사람",
    grp("1. 우리, 결혼할 사주인가요?") +
    fl(1, "사주에서 보이는 결혼 시기", marryPick.length ? `두 사람의 혼인 별이 겹치는 해는 <strong>${marryPick.map(y => y.year + "년").join(" · ")}</strong>이에요. 이 해의 혼담, 상견례, 결정에는 하늘의 협조가 붙는답니다.` : "앞으로 6년 안에 강한 혼인운의 해가 뚜렷하진 않아요 — 늦는 게 아니라 서두를 이유가 없다는 뜻이에요.") +
    fl(2, "결혼의 장애물", (br.k === "chung" || br.k === "wonjin" ? `배우자궁의 ${br.label} — 생활 방식의 정면충돌이 최대 관문이에요. 결혼 전에 동거 수준의 생활 리허설(여행, 살림 분담)을 꼭 해 보시길 권해요.` : zr.k === "chung" || zr.k === "wonjin" ? "두 집안의 문화 차이가 관문이에요. 두 사람의 문제가 아니니까, '부부는 한 팀'이라는 원칙만 세우면 넘을 수 있어요." : "큰 장애물은 안 보여요 — 오히려 '완벽한 때'를 기다리는 마음이 유일한 장애물이에요.")) +
    fl(3, "신혼생활은 어디서 시작하는 게 좋을까?", `${yA.yongE === yB.yongE ? `두 분 모두 ${DIR[yA.yongE]}이 길방이라 그 방향이 정답이에요.` : `${A.name}님의 길방(${DIR[yA.yongE]})과 ${B.name}님의 길방(${DIR[yB.yongE]}) 사이에서 고르시되, 창이 그 방향으로 난 집이면 충분해요.`}`) +
    fl(4, "가사분담은 어떻게?", `살림의 감각이 있는 <strong>${lateGiver}님</strong>이 주방과 일상을, 추진력 있는 <strong>${leader}님</strong>이 행정과 큰일(이사·계약·수리)을 맡는 그림이 자연스러워요.`) +
    grp("2. 이혼 위기 극복 방법") +
    fl(1, "우리에게 이혼 위기가 있을까?", br.k === "chung" || br.k === "wonjin" ? "솔직하게 말씀드릴게요 — 흔들리는 시기가 와요. 다만 이 배합의 위기는 애정이 식어서가 아니라 생활 충돌이 쌓여서라, 예방이 가능해요." : "배합상 큰 위기의 그림은 없어요. 이 커플의 적은 위기가 아니라 무심의 축적이에요.") +
    fl(2, "위기가 찾아올 시점", crisisYears.length ? `배우자궁이 흔들리는 <strong>${crisisYears.slice(0, 2).join("년, ")}년</strong> 언저리예요 — 이사나 이직 같은 환경 변화와 겹치면 체감이 커져요.` : "특정 연도보다, 큰 환경 변화(출산·이직·이사) 직후 6개월이 위험 구간이에요.") +
    fl(3, "위기가 오는 이유", `위 감정궁합에서 짚은 서운함 포인트(${CAUTIONS[A.ds][0].split("—")[0]} / ${CAUTIONS[B.ds][0].split("—")[0]})가 말해지지 않고 쌓이는 것 — 그게 이 커플 위기의 8할이에요.`) +
    fl(4, "슬기롭게 극복하려면", "위기의 해에는 큰 결정을 미루고, 부부 회의를 주 1회 고정해 보세요. 그리고 기억해 주세요 — 충(沖)의 해는 헤어지는 해가 아니라 '재계약 조건을 갱신하는 해'예요.") +
    grp("3. 우리 둘의 자녀운") +
    fl(1, "자녀 계획", kids >= 3 ? "두 사람의 자녀별이 넉넉해요 — 아이와의 연이 자연스럽게 이어지는 배합이에요." : kids >= 1 ? "자녀별이 은은하게 자리해요 — 계획을 세우면 순리대로 풀리는 그림이에요." : "자녀별이 겉으로 드러나지 않은 배합이에요 — 연이 없다는 뜻이 아니라 시기와 준비가 중요하다는 뜻이니까, 서두르기보다 몸과 기반을 먼저 만들어 두세요.") +
    fl(2, "자녀운이 약할 때 보완법", `두 사람의 용신(${ELEM[yA.yongE]}·${ELEM[yB.yongE]})을 침실에 — 색과 소품으로 채우고, 몸을 데우는 음식(${FOODS[yA.yongE]})을 함께 챙겨 보세요.`) +
    fl(3, "우리에게 맞는 자녀 교육", (dominantTG(A) === "정인" || dominantTG(B) === "정인" ? "인성의 부모예요 — 차분히 가르치는 학습형 교육이 강점이에요. 다만 아이의 실수를 기다려 주는 연습도 함께요." : "식상의 감각이 있는 부모예요 — 몸으로 겪게 하는 체험형 교육이 맞아요. 학원보다 여행과 프로젝트를요.")) +
    fl(4, "양가 가족과의 궁합", `${zr.k === "yukhap" || zr.k === "samhap" ? "띠의 합이 좋아서 양가 어른들과의 관계가 순탄한 편이에요." : zr.k === "chung" || zr.k === "wonjin" ? "띠가 부딪혀서 명절이나 가족 행사에서 잔가시가 돋기 쉬워요. 서로의 원가족 얘기는 농담으로도 조심해 주세요." : "양가와의 관계는 무난한 흐름이에요 — 예의만 지키면 탈이 없어요."}`) +
    fl(5, "명절·가족 행사 갈등 줄이는 법", `<ul class="pts"><li>배우자를 내 가족 앞에서 무조건 편들기 — 중재는 집에 와서 하세요.</li><li>양가 방문과 용돈은 '동일 규칙'으로 — 형평이 곧 평화예요.</li><li>가족 모임 전 10분, 오늘의 지뢰(피할 화제)를 서로 브리핑하세요.</li></ul>`),
...
export function renderYearly(A, job = "직장인") {
  ...
  if (!sy) return sec("一", "올해의 운세", "", "<p>올해 세운 정보를 계산할 수 없어요.</p>");
  ...
  html += sec("第一章", "올해의 총운", `${thisYear}년 ${sy.ganzhi}년 · 세운 ${mainTG}(천간)·${subTG}(지지)${sjStage >= 0 ? " · " + ["들", "눌", "날"][sjStage] + "삼재" : ""}`,
    baseChartSec(A) +
    fl(1, `${thisYear}년 주요 특징과 변화`, `<p>${first(YEAR_TG[mainTG] ?? "", 2)}</p><p>발밑으로는 ${subTG}의 저류도 함께 흘러요 — ${SEYUN_TG[subTG] ?? ""}.</p>`) +
    fl(2, "올해 전반적인 흐름", yukhapM.concat(goodMoneyM).filter(m => m <= 6).length >= yukhapM.concat(goodMoneyM).filter(m => m > 6).length
      ? "상반기에 순풍이 몰려 있어요 — 중요한 시작과 결정은 상반기에 싣고, 하반기는 수확과 정리에 쓰세요."
      : "하반기로 갈수록 힘이 붙는 흐름이에요 — 상반기는 준비와 축적, 승부는 하반기에 거세요.") +
    fl(3, "나의 올해 키워드는?", `<strong>"${keyword}"</strong> — 이 문장을 거스르는 계획일수록 올해는 마찰이 커져요.`) +
    fl(4, "찾아올 수 있는 위기는?", `${chungM.length ? `<strong>${chungM.map(m => m + "월").join("·")}</strong> — 배우자궁이 흔들리는 달이라 관계·계약의 변동수가 있어요. ` : ""}${MONTH_CAUTION[mainTG]} — 올해 유형의 위기예요.${sjStage >= 0 ? ` 그리고 ${["들", "눌", "날"][sjStage]}삼재 구간이니까, ${SAMJAE_STAGE[sjStage].split("—")[1]?.trim()}` : ""}`) +
    fl(5, "얻을 수 있는 기회는?", `${yukhapM.length ? `<strong>${yukhapM.map(m => m + "월").join("·")}</strong>은 인연과 계약이 몸에 붙는 육합의 달이에요. ` : ""}희신(${ELEM[heeE]})의 기운이 도는 자리 — ${ELEM_PERSON[heeE].split("—")[0].trim()}과의 협업이 올해 최고의 기회 루트예요.`),
  ...
  html += sec("第二章", "재물운", "",
    ...
    fl(2, "실제 일어날 수 있는 사건", `${MONTH_CAUTION[mainTG]}. 이런 종류의 일이 올해 돈 문제의 전형적인 얼굴이에요.`) +
    fl(3, "내게 필요한 조언은?", MONEY_TG[subTG] ? `지지의 보조 흐름도 참고하세요 — ${MONEY_TG[subTG]}` : "수입보다 지출 구조를 먼저 점검해 보세요.") +
    grp("2. 월별 재물운과 사람") +
    fl(1, "금전운 상승기", goodMoneyM.length ? `<strong>${goodMoneyM.map(m => m + "월").join(" · ")}</strong> — 재성과 식신이 흐르는 달이에요. 협상, 이직, 부수입 시도는 이 달에 하세요.` : "특정 상승월보다 꾸준함이 유리한 해예요.") +
    fl(2, "조심해야 할 시기", badMoneyM.length ? `<strong>${badMoneyM.map(m => m + "월").join(" · ")}</strong> — 비견·겁재의 달이라 지출과 분배 손실이 커지기 쉬워요. 빌려주는 돈, 동업 제안은 이 달만은 보류하세요.` : "크게 조심할 달은 없는데, 충동 지출은 상시 경계예요.") +
    fl(3, "올해 이익을 가져다줄 사람", `희신 ${ELEM[heeE]}의 사람이에요 — ${ELEM_PERSON[heeE]}.`) +
    fl(4, "올해 손해를 가져다줄 사람", `기신 ${ELEM[giE]}의 사람이에요 — ${ELEM_PERSON[giE]}. 특히 돈과 문서가 얽힌 부탁은 정중히 거절하세요.`) +
    fl(5, "조언", "올해 재물의 핵심은 '누구와'예요. 금액이 큰 결정일수록 위의 두 사람 유형을 대입해 보세요."),
    (MONEY_TG[mainTG] ?? "").split("—")[0].replace("재물은", "").trim());

  html += sec("第三章", "건강운", "",
    grp("1. 건강운 총운") +
    fl(1, "나의 체질과 건강은?", first(HEALTH_BY_ELEM[maxE], 2)) +
    fl(2, "조심해야 할 부상·질병", `부족한 ${ELEM[minE]}의 영역이에요 — ${ORGANS[minE]} 쪽을 챙겨 주세요. ${chungM.length ? `변동의 달(${chungM.map(m => m + "월").join("·")})에는 과로와 사고수도 함께 오니까 무리한 일정은 금물이에요.` : ""}`) +
    fl(3, "생활습관과 활동", `용신 ${ELEM[yongE]}을 채우는 생활이 좋아요 — ${["아침 산책과 스트레칭", "낮의 야외 활동과 웃는 자리", "규칙적 식사와 정돈된 공간", "근력 운동과 호흡 운동", "충분한 수분과 밤의 숙면"][yongE]}이 올해의 보약이에요.`) +
    fl(4, "식습관 추천", `${FOODS[yongE]}을 가까이, ${FOODS[giE]}은 과하지 않게요.`) +
    grp("2. 올해의 추천 운동") +
  ...
  html += sec("第四章", "애정운", "",
    grp("1. 애정운 총운") +
    fl(1, "올해 나의 애정운은?",
      `<p><strong>연애 중·기혼이라면</strong> — ${rel.k === "chung" ? "배우자궁이 충으로 흔들리는 해예요. 관계의 문제가 커지는 게 아니라 '드러나는' 해니까, 올해 나온 갈등은 오히려 수리의 기회로 쓰세요." : rel.k === "yukhap" ? "배우자궁에 합이 드는 해예요 — 관계가 깊어지고 결실(동거·약혼·2세) 이야기가 자연스러워져요." : "관계의 큰 파도는 없는 해예요 — 안정 속에서 서로의 일상을 지켜 주는 게 최고의 애정 표현이에요."}</p>` +
      `<p><strong>솔로·미혼이라면</strong> — ${loveHit ? `배우자별(${starName(A)})이 드는 해예요! 올해의 소개와 만남은 평년보다 무게가 다르니까 가볍게 넘기지 마세요.` : "배우자별이 직접 드는 해는 아니라서, 인연은 사건보다 일상으로 스며들어요. 아래 길한 달에 집중하세요."}</p>`) +
    fl(2, "애정운 상승 시기와 장소",
      `<p><strong>시기</strong> — ${loveM.length ? loveM.map(m => m + "월").join(" · ") : "연말로 갈수록"}이에요.</p><p><strong>장소</strong> — ${first(LOVER_PROFILE[partnerElem(A)].where, 1)}</p>`) +
    grp("2. 애정운 개선 방법") +
    fl(1, "올해 나의 매력은?", monthsWhere(A, mb => mb === DOHWA[db]).length ? `도화가 발동하는 ${monthsWhere(A, mb => mb === DOHWA[db]).map(m => m + "월").join("·")}에 매력이 정점을 찍어요 — 프로필 사진, 소개팅, 발표는 이때 하세요.` : `${FIRST_IMPRESSION[ds]} — 이 첫인상이 올해의 무기예요.`) +
    fl(2, "애정운 상승을 위한 조언", `${CAUTIONS[ds][0]} — 올해만큼은 이 습관을 의식적으로 접어 두세요. 애정운의 절반이 여기서 갈려요.`) +
    fl(3, "내게 맞는 행운의 아이템", `${COLOR[yongE]} 계열의 소품이에요. 데이트나 소개 자리는 ${DIR[yongE]} 방향의 장소가 길하고요.`),
    loveHit ? `배우자별이 드는 해 — 올해의 만남은 무게가 다르다` : `길한 달 ${loveM.slice(0, 2).map(m => m + "월").join("·") || "연말"} — 일상 속 스며드는 인연`);

  html += sec("第五章", "직장·명예운", job ? `현재 상태: ${job}` : "",
    grp("1. 직장·명예운 총운") +
    fl(1, "올해 나의 사회적 위치와 평판은?", `${WORK_TG[mainTG] ?? ""} ${["정관", "편관"].includes(mainTG) || ["정관", "편관"].includes(subTG) ? "관성이 드는 해라 직함·평판·책임이 함께 커져요." : ""}`) +
    fl(2, "현명한 사회생활을 위한 조언", `${WORK_TG[subTG] ?? ""} ${isStudent ? `${job}인 지금은 이 흐름을 '시험과 준비의 운'으로 읽으세요 — 제6장 학업운이 올해 당신의 본진이에요.` : job === "사업·자영업" || job === "프리랜서" ? "조직이 아닌 내 판을 가진 당신에게 명예운은 곧 평판 마케팅이에요 — 후기를 쌓는 해로 쓰세요." : ""}`) +
    fl(3, "내게 도움이 될 사람", `${ELEM_PERSON[heeE]} — 희신의 사람이에요. 올해 상사, 동료, 파트너 중 이 유형을 가까이하세요.`) +
    fl(4, "반드시 피해야 할 사람", `${ELEM_PERSON[giE]} — 기신의 사람이에요. 험담의 자리, 보증·연대 책임은 이 유형과 얽히지 마세요.`) +
    grp("2. 월별 커리어 흐름") +
    fl(1, "중요한 결정, 언제가 좋을까?", docGoodM.length ? `<strong>${docGoodM.map(m => m + "월").join(" · ")}</strong> — 정관과 정인이 흐르는 달이라 이직, 계약, 시험, 발표에 길해요.` : "올해는 결정보다 실력을 쌓는 해로 쓰는 게 유리해요.") +
    fl(2, "주의해야 하는 시기", docBadM.length ? `<strong>${docBadM.map(m => m + "월").join(" · ")}</strong> — 상관의 달이라 윗사람과의 마찰, 구설을 조심하세요. 사표와 항의 메일은 이 달만은 보류!` : "특별히 주의할 달은 없어요.") +
    fl(3, "커리어 향상을 위한 조언", `${(DAEUN_TG[r.daeun.current?.stemTenGod] ?? "").split("—")[0].trim()} 대운 위에 올해가 얹혀 있어요 — 올해의 선택은 이 10년 흐름과 같은 방향일 때 두 배로 커진답니다.`),
  ...
  html += sec("第六章", "학업·계약운", isStudent ? "올해 당신의 본진이에요" : "",
    grp("1. 합격운 총운") +
    fl(1, "올해의 합격운은?", `${insung >= 2 ? "원국에 인성(학문의 별)이 든든해서 시험과 평가에 강한 체질이에요." : "인성이 강한 사주는 아니지만 괜찮아요 — 합격은 체질보다 시기 선택이 좌우하거든요."} 올해는 ${["정인", "정관"].includes(mainTG) || ["정인", "정관"].includes(subTG) ? "문서와 명예의 별이 드는 해라 합격운이 밝은 편이에요." : "합격의 별이 강하게 드는 해는 아니니까, 아래 길한 달에 시험 일정을 맞추는 전략이 필요해요."}`) +
    fl(2, "합격운이 좋은 달은?", docGoodM.length ? `<strong>${docGoodM.map(m => m + "월").join(" · ")}</strong> — 시험, 면접, 지원서 제출을 이 달에 맞춰 보세요.` : "특정 길월보다 꾸준한 축적이 답인 해예요.") +
    fl(3, "주의가 필요한 달은?", `${docBadM.length ? `<strong>${docBadM.map(m => m + "월").join(" · ")}</strong> — 실수와 구설의 달이에요. 시험이 이 달이라면 검토 시간을 평소의 두 배로 잡으세요.` : "없어요!"}`) +
    fl(4, "어떤 것들을 점검해야 할까?", `<ul class="pts"><li>서류·자격 요건의 사소한 누락 (${MONTH_CAUTION["정관"]})</li><li>컨디션 관리 — ${ELEM[minE]} 영역(${ORGANS[minE]})이 시험기에 흔들리기 쉬워요</li><li>마감 하루 전 제출 원칙 — 미루는 버릇이 최대의 적이에요</li></ul>`) +
    fl(5, "합격운 상승 행운의 아이템", `${COLOR[yongE]} 계열의 필기구나 소품 — 시험장과 면접장 책상 위의 부적이에요.`) +
    grp("2. 계약운 총운") +
    fl(1, "올해의 계약운은?", `${["정인", "정재"].includes(mainTG) ? "문서와 실속의 별이 함께 드는 해라 계약운이 밝아요." : "계약 자체보다 계약 조건이 중요한 해예요."} 부동산, 이직, 투자 문서 모두 아래 달을 참고하세요.`) +
    ...
    fl(3, "계약 전 점검과 개운법", `도장 찍기 전 세 번 확인은 기본이고요 — 계약 자리는 ${DIR[yongE]} 방향, ${COLOR[yongE]} 소지품이 길하답니다.`),
  ...
  html += sec("第七章", "12개월 월별 운세", "달마다 기회·주의·한 문장",
    r.wolun.map(w => {
      ...
      const mark = mr.k === "yukhap" ? " · <em>육합의 달(관계·계약에 길해요)</em>" : mr.k === "chung" ? " · <em>충의 달(변동 주의!)</em>" : mb === DOHWA[db] ? " · <em>도화의 달(매력 상승)</em>" : "";
      return `<div class="fl"><p class="fl-t">${mm}월 <small>(${w.monthName.slice(0, 2)} · ${w.branchTenGod})</small></p><div class="fl-b">
        ...
        <p><strong>이달의 한 문장</strong> — "${(MONTH_TG[w.branchTenGod] ?? "").split("—")[1]?.trim() ?? "평상심이 길이에요"}"</p>
      </div></div>`;
    }).join(""),
    `가장 밝은 달 ${yukhapM.concat(docGoodM)[0] ?? goodMoneyM[0] ?? "-"}월 · 가장 조심할 달 ${chungM[0] ?? docBadM[0] ?? "-"}월`);

  html += sec("終章", "길흉육조(吉凶六條)", `${thisYear}년을 여섯 문장으로 정리해 드릴게요`,
    `<p><strong>길(吉) 세 가지</strong>예요.</p><ul class="pts gold">
      <li>${yukhapM.length ? `${yukhapM.map(m => m + "월").join("·")}의 육합 — 관계와 계약이 몸에 붙는 시기예요` : `희신 ${ELEM[heeE]}의 사람 — 올해의 귀인 루트예요`}</li>
      <li>${keyword} — 이 흐름에 올라탄 계획은 순풍을 받아요</li>
      <li>용신 ${ELEM[yongE]} — ${COLOR[yongE]}과 ${DIR[yongE]}이 일 년 내내 나의 부적이에요</li>
    </ul>
    <p><strong>흉(凶) 세 가지</strong>도 솔직하게요.</p><ul class="pts">
      <li>${chungM.length ? `${chungM.map(m => m + "월").join("·")}의 충 — 이 달의 변동과 감정적 결정은 하루 재우세요` : `기신 ${ELEM[giE]}의 사람 — 만나면 방전되는 관계를 경계하세요`}</li>
