...
export function renderSaju(A) {
  ...
  const strengthBody =
    ...
    `<p>${s.cls === "strong"
      ? `${basis.join(", ")} 기운이 강한 사주예요. 추진력·독립심이 강점이라 남 밑보다 <strong>내가 주도하는 자리</strong>(창업·전문직·팀장)가 잘 맞아요. 단점은 '내 방식' 고집이라 부딪힘이 잦은 것 — 넘치는 힘을 일·운동·목표로 <em>써서 빼는</em> 게 균형이에요.`
      : s.cls === "weak"
        ? `${noBasis.join(", ")} 기운이 여린 사주예요. 혼자 다 짊어지기보다 <strong>좋은 판을 고르는 것</strong>이 전략이라, 프리랜서보다 소속, 단독보다 팀·멘토·좋은 회사 위에서 실력이 배가돼요. 거절 못 하고 떠안는 습관만 고치면 훨씬 편해져요.`
        : `기운이 넘치지도 모자라지도 않은 균형형이에요. 상황 따라 밀고 당기는 <strong>조절력</strong>이 강점이라, 이끄는 자리와 맞추는 자리 양쪽에 다 잘 적응해요.`}</p>` +
...
export function renderLoveSecret(A, relStatus = 0, relGap = 0) {
  ...
  html += sec("第四章", "내 운명의 짝은?", `${sName} = ${ELEM[pe]}의 사람`,
    ...
    fl(3, "구체적으로 어떤 사람인지",
      ...
      note(`왜 ${ELEM[pe]}이냐고요? ${A.sex === "F" ? "여성의 배우자별은 나를 이끄는 관성(官星)" : "남성의 배우자별은 내가 아끼는 재성(財星)"}인데, 당신의 일간 ${STEMS[ds].kr}(${STEMS[ds].hj}) 기준으로 ${ELEM[pe]}이 바로 그 별이거든요.`)),
